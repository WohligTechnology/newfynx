...
//    }
    ...
    public function getImageForCustomize()
    {
        ...
        $query->color = $this->db->query("SELECT DISTINCT `fynx_color`.*,`fynx_product`.`id` as `product`,`fynx_product`.`price` as `price` FROM `fynx_product` INNER JOIN `fynx_color` ON `fynx_color`.`id` = `fynx_product`.`color`  WHERE `type`='$type' GROUP BY `fynx_size`.`id`")->result();
        $data['message'] = $query;
        $this->load->view('json', $data);
    }
}
